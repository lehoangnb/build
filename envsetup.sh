...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^vega_") ; then
       VEGA_BUILD=$(echo -n $1 | sed -e 's/^vega_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $VEGA_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       VEGA_BUILD=
    fi
    export VEGA_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    VEGA_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/vega/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the VEGA model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch vega_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the VEGA github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var VEGA_VERSION)
        ZIPFILE=vega-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.vega.device | grep -q "$VEGA_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $VEGA_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.vega.device | grep -q "$VEGA_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/vega/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
